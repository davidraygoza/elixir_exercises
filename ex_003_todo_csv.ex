defmodule MinimalTodo do
  def start do
    input = IO.gets("Would you like tocreate a new .csv? (y/n)\n")
    |>String.trim
    |>String.downcase
    if input == "y" do
  ...
  def create_headers do
    ...
    <> "Enter field names one by one an empty line when you're done. \n"
    create_header([])
  ...
  def delete_todo(data) do
    todo = IO.gets("Wich todo would like to delete?\n") |> String.trim()
  ...
  def get_item_name(data) do
    ...
    if Map.has_key?(data, name) do
      IO.puts("Todo with that name already exist! \n")
      get_item_name(data)
    else
      name
    end
  ...
  def save_csv(data) do
    filename = IO.gets("Name pf .csv to save") |> String.trim()
